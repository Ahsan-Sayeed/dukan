...
const Card = ({pro}: Props) => {
    const pathname = usePathname();
    return (
        <div className={`stats shadow mx-2 bg-base-100 shadow-xl flex border h-52 ${pathname==='/users'?"w-full":"w-56"}`}>
            <div className="card-body">
                <h2 className="stat-title">{pro}</h2>
                <div className='flex items-center'>
                    <p className='stat-value text-secondary'>120</p>
                    {pathname === '/users' && <div className="radial-progress text-primary">5%</div>}
                </div>
